...
<!DOCTYPE html>
 ...
 <p>・<a href = "http://co-412.it.99sv-coco.com/japan-support.php">日本代表応援掲示板ページ</a></p>
 ...
 <form action = "worldcup.php" method = "post" enctype="multipart/form-data">
 ...
 <form action = "worldcup.php" method = "post">
...
<?php
 ...
 try{
          ...
          //SQLコマンド「CREATE TABLE」で新規テーブル"comments"を作成する
 ...
 if(isset($_POST["submit"])){
          ...
          if( (!empty($name)) && (!empty($comment)) && (!empty($pass)) && (empty($file)) ) {
                    ...
                    $sql = $pdo -> prepare("INSERT INTO ".$table."(name,comment,time,pass) VALUES(:name, :comment, :time, :pass)");
...
/*****削除フォームのコメント*****/
 ...
 if( (isset($_POST["delete"])) && (!empty($delete_num)) ){
          ...
          if ( $delete_pass==$right_pass ) {
                    ...
                    echo "<form action = 'worldcup.php' method = 'post'>";
 ...
 if(isset($_POST["submit"])){
          ...
          if( (!empty($name)) && (!empty($comment)) && (!empty($pass)) ) {
                    ...
                    if (is_uploaded_file($_FILES["upfile"]["tmp_name"])) {
                              ...
                              if ( move_uploaded_file($_FILES["upfile"]["tmp_name"], "./wcfiles/". $_FILES["upfile"]["name"]) ) {

                                        $file_path = "./wcfiles/". $_FILES["upfile"]["name"];
